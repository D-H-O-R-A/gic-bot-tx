...
import {sendTransactionsLoopToMe, sendTransactionsLoopRandom,sendTransactionsLoopToAnother} from "../tools/web3.js"
...
function cliInitial() {
    ...
    console.log("setchainisd - Define a Chain ID (saved in .env)");
    ...
    console.log("starttome - Same configuration requirements as start, but this mode sends transactions to the same address configured in .env")
    console.log("starttomeandramdom - Same configuration requirements as start, but this mode sends transactions to the same address configured in .env and random address")
    console.log("starttoanother - Same configuration requirements as start, but this mode sends transactions to another address.")
    console.log("setminimuntransfer - Set the minimum transfer value (in Wei) for the transaction. Default is 0.01 GIC")
    ...
    rl.on("line", async (input) => {
        ...
        switch (command) {
            case "setminimuntransfer":
                const minimunTransfer = parseInt(value, 10);
                if (isNaN(minimunTransfer) || minimunTransfer <= 0) {
                    console.log("Error: Set a valid value greater than 0.");
                } else {
                    saveEnvVariable("MINIMUN_TRANSFER", minimunTransfer*(10**18));
                }
                break;
            case "setseed":
                if (!value) {
